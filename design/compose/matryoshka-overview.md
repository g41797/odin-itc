...
All rules live here.  
When this file says something different from any other file — this file wins.
...
## Doll 3 — Mailbox (simple blocking)
...
Main thread reads file → gets chunk from pool → sends  
Worker thread waits → receives → compresses → sends progress and result back  
Main thread receives progress (updates bar) and result (writes file)
